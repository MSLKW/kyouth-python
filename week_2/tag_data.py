...
def batch_update(conn: sqlite3.Connection, model: str, batch_amount: int):
    ...
    while True:
        ...
        batch_index += 1
        prompt = """Gather the technology stack from the dataset provided and present it in a list formatted as (source_id:tech_stack) separated by ", ".
        ...
        try:
            result = prompt_model(model=model, prompt=prompt)
